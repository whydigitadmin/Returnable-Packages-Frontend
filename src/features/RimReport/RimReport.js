...
export const RimReport = () => {
  ...
  useEffect(() => {
    getPendingBinRetrievalFromRIM();
  }, []);

  const getPendingBinRetrievalFromRIM = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/master/getPendingBinRetrievalFromRIM?orgId=${orgId}`
      );

      if (response.status === 200) {
        setTableData(
          response.data.paramObjectsMap.retrievalManifestProviderVOs
        );
      }
    } catch (error) {
  ...
  const handleCreateIssueRequest = async () => {
    ...
    const baseUrl = "/api/master/createBinRetrievalFromRIM";
    const createdBy = userName;
    const rimIdParams = mimIds.map((id) => `rimIds=${id}`).join("&");
    const fullUrl = `${process.env.REACT_APP_API_URL}${baseUrl}?createdBy=${createdBy}&${rimIdParams}`;

    try {
      const response = await axios.post(fullUrl);

      if (response.status === 200) {
        console.log("processedRequests", response.data.paramObjectsMap.details);
                ...
                <table className="table table-hover w-full">
                  ...
                  <tbody>
                    {tableData && tableData.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan={9}>
                          <NoRecordsFound message={"RIM Report Not Found..!"} />
                        </td>
                      </tr>
                    )}
                  </tbody>
