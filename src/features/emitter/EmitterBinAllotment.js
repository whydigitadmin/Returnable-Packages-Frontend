...
function EmitterBinAllotment({ addBinAllotment, editBinRequestId }) {
    ...
    const getBinRequestByReqNo = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                console.log("GET ONE BINREQ DETAILS FROM API Response:", response.data.paramObjectsMap.BinAllotment);
    ...
    const viewAllotedBinByDocId = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                setStockFrom(response.data.paramObjectsMap.binAllotmentNewVO[0].stockBranch);
    ...
    const getAvlQtyByBranch = async () => {
        try {
            const response = await axios.get(
                `${process.env.REACT_APP_API_URL}/api/master/getAvalkitqtyByBranch?branch=${stockFrom}&Kitname=${reqKitName}`
            );
