...
function WarehouseLocation() {
  ...
  const getWarehouseData = async () => {
    try {
      ...
      if (response.status === 200) {
        setData(response.data.paramObjectsMap.warehouseloactionVO);
        setTableData(response.data.paramObjectsMap.warehouseloactionVO);
      }
    } catch (error) {
  ...
  const handleWarehouse = () => {
    ...
    if (!warehouselocation) {
      errors.warehouselocation = "Unit Name is required";
    }
    if (Object.keys(errors).length === 0) {
      ...
      axios
        .post(
          `${process.env.REACT_APP_API_URL}/api/master/warehouselocation`,
          formData
        )
        .then((response) => {
        ...
        <div className="row">
          ...
          <div className="col-lg-3 col-md-6 mb-2">
            ...
            {errors.warehouselocation && (
              <div className="error-text">{errors.warehouselocation}</div>
            )}
          </div>
